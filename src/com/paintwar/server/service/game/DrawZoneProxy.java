			...
			for (Entry<String, DrawingServerProxy> currentDrawItem : drawings.entrySet()) {
				if (!currentDrawItem.getKey().equals(name)) {
					...
					if (newDrawing.getBox().intersects(currentDraw.getBox())) {
						//if intersects and same color
						if (newDrawing.getColor().equals(currentDraw.getColor())) {
							...
							if (newP2.x != oldP2.x) {
								//expanding horizontally, setting oldP2 to be the other angle of the rectangle
							...
							addedArea.add(newP2);
							Logger.print("areaAdded " + addedArea.height*addedArea.width);
							
							//generating area to not take in account
							Rectangle currentAreaToNotCount = currentDraw.getBox().intersection(addedArea); //TODO missing pixels here, resulting in ignoring more than expected
							if (!currentAreaToNotCount.isEmpty()) {
								Logger.print("ignoring " + currentAreaToNotCount.height*currentAreaToNotCount.width);
								currentDraw.addRemovedArea(new Area(currentAreaToNotCount)); //removing this area to count
								scoreToIgnore += currentAreaToNotCount.height*currentAreaToNotCount.width;
							}
